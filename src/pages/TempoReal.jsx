...
export function TempoReal() {
  ...
  // ✅ CORREÇÃO PRINCIPAL: mover update de contexto para useEffect
  useEffect(() => {
    if (usuarioEncontrado && negocioDoUsuario.length && negocios.length) {
      const relacao = negocioDoUsuario.find(
        (negocio) => negocio.userId === usuarioEncontrado.id
      );
      const negocio = negocios.find(
  ...
  }, [usuarioEncontrado, negocioDoUsuario, negocios]);
